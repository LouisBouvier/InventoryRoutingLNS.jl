...
function read_instance(path::String)::Instance
    ...
    commodities = [read_commodity(data[1+m], dims) for m = 1:dims.M]
    depots = [read_depot(data[1+dims.M+d], dims) for d = 1:dims.D]
    customers = [read_customer(data[1+dims.M+dims.D+c], dims) for c = 1:dims.C]
    dist = read_distances(data[1+dims.M+dims.D+dims.C+1:end], dims)
    transport_durations = read_transport_durations(data[1+dims.M+dims.D+dims.C+1:end], dims)
...
function read_solution(path::String)::SimpleSolution
    ...
    routes = [read_route(sol[1+r]) for r = 1:R]
...
function read_commodities_CSV(path::String)
    ...
    return [Commodity(m = m, l = lengths[m]) for m = 1:length(lengths)], names
end
...
function read_depots_CSV(path::String, dims::NamedTuple, commodity_codes::Vector{String})
    ...
    for depot_code in depots_codes
        ...
        ## per commodity
        df_per_commodity =
            DataFrame(CSV.File(joinpath(path_to_depot_folder, "per_commodity.csv")))
        excess_inventory_cost = Vector{Int}(undef, dims.M)
        initial_inventory = Vector{Int}(undef, dims.M)
        @assert(commodity_codes == df_per_commodity.commodity_code)
        for m = 1:length(commodity_codes)
...
function read_customers_CSV(
    ...
    for customer_code in customer_codes
        ...
        df_customer_global =
            DataFrame(CSV.File(joinpath(path_to_customer_folder, "global.csv")))
        ...
        ## per commodity
        df_per_commodity =
            DataFrame(CSV.File(joinpath(path_to_customer_folder, "per_commodity.csv")))
        excess_inventory_cost = Vector{Int}(undef, dims.M)
        initial_inventory = Vector{Int}(undef, dims.M)
        shortage_cost = Vector{Int}(undef, dims.M)
        @assert(commodity_codes == df_per_commodity.commodity_code)
        ...
        df_max_inventory =
            DataFrame(CSV.File(joinpath(path_to_customer_folder, "maximum_stock.csv")))
        maximum_inventory = Matrix(select!(df_max_inventory, Not(:commodity_code)))
...
the sites indices used when reading depots and customers.
"""
function read_transport_durations_CSV(path::String, dims::NamedTuple, v_index_to_code::Vector{Int})
...
function read_instance_CSV(path_to_folder::String)::Instance
    ...
    customers, v_index_to_code =
        read_customers_CSV(path_to_folder, dims, commodities_names, v_index_to_code)
