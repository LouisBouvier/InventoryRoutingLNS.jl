...
function insert_multi_depot!(
    ...
    if !isempty(route_mod1.stops)
        ...
        oldcost = compute_cost(instance, ds = ds, cs = cs, ms = ms, ts = ts, solution = SimpleSolution([route1, route2]))
        ...
        if feasibility(instance, ds = [route2.d], cs = Vector{Int}(), solution = SimpleSolution([route_mod2]))
        ...
        if feasibility(instance, ds = [route2.d], cs = Vector{Int}(), solution = SimpleSolution([route_mod2]))
            newcost = compute_cost(
                ...
                ms = ms,
                ts = ts,
                solution = SimpleSolution([route_mod2]),
            )
...
function swap_multi_depot!(
    ...
    if content_size(route_mod1, instance) > instance.vehicle_capacity || content_size(route_mod2, instance) > instance.vehicle_capacity 
        stats["swap_multi_depot_aborted"] += 1
        return false
    end
...
function iterative_insert_multi_depot!(instance::Instance, t::Int; stats::Dict = nothing)
    ...
    for it = 1:nb_it
        ...
        for i = 1:length(route1.stops)
            improvement_achieved = insert_multi_depot!(instance, route1, route2, i, stats = stats)
...
function iterative_swap_multi_depot!(instance::Instance, t::Int; stats::Dict = nothing)
    ...
    for it = 1:nb_it
        ...
        route1 = get_route_day(instance.solution, t, r1)
        route2 = get_route_day(instance.solution, t, r2)
        for i = 1:length(route1.stops), k = 1:length(route2.stops)
            improvement_achieved =
                swap_multi_depot!(instance, route1, route2, i, k, stats = stats)
            if improvement_achieved
                break
            end
        end
...
function insert_swap_multi_depot_per_day!(instance::Instance; stats::Dict = nothing)
    T = instance.T
    @showprogress "Insert and swap per day: " for t = 1:T
...
function two_opt_star_multi_depot!(
    ...
    newroute1_begin, newroute1_end = mycopy(route1.stops[1:k]), mycopy(route2.stops[l+1:end])
    newroute2_begin, newroute2_end = mycopy(route2.stops[1:l]), mycopy(route1.stops[k+1:end])
    newroute1.stops = vcat(newroute1_begin, newroute1_end)
    newroute2.stops = vcat(newroute2_begin, newroute2_end)

    if get_nb_unique_stops(newroute1) > instance.S_max || get_nb_unique_stops(newroute2) > instance.S_max
        stats["two_opt_star_multi_depot_aborted"] += 1
        return false
    end

    if content_size(newroute1, instance) > instance.vehicle_capacity || content_size(newroute2, instance) > instance.vehicle_capacity
        stats["two_opt_star_multi_depot_aborted"] += 1
        return false
    end
    ...
    ts = collect(route1.t:T)
    ds = unique(route.d for route in [route1, route2])
    cs = unique(vcat([stop.c for stop in route1.stops[k+1:end]], [stop.c for stop in route2.stops[l+1:end]])) 
    ms = [m for m = 1:M if (uses_commodity(route1, m) || uses_commodity(route2, m))]
    # Compute old cost
    oldcost = compute_cost(instance, ds = ds, cs = cs, ms = ms, ts = ts, solution = SimpleSolution([route1, route2]))
    update_instance_some_routes!(instance, [route1, route2], "delete", false)
...
"""
function iterative_two_opt_star_multi_depot!(instance::Instance, t::Int; stats::Dict = nothing)
    R = nb_routes(instance.solution, t)
    for r1 = 1:R, r2 = 1:R
        ...
        route1 = get_route_day(instance.solution, t, r1)
        route2 = get_route_day(instance.solution, t, r2)
        for k = 1:(length(route1.stops)-1), l = 1:(length(route2.stops)-1)
            improvement_achieved = two_opt_star_multi_depot!(instance, route1, route2, k, l, stats = stats)
            if improvement_achieved
                break
            end
        end
...
function two_opt_star_multi_depot_per_day!(instance::Instance; stats::Dict = nothing)
    T = instance.T
    @showprogress "Two_opt_star_multi_depot per day: " for t = 1:instance.T
