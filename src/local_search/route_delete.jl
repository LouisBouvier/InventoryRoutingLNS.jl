...
function delete_non_profitable_routes!(instance::Instance, t::Int, d::Int)
    ...
    while room_for_improvement
        ...
        for route in sort(
            ...
            oldcost =
                compute_cost(instance, ds = ds, cs = cs, solution = SimpleSolution([route]))
            update_instance_some_routes!(instance, [route], "delete")
            newcost =
                compute_cost(instance, ds = ds, cs = cs, solution = SimpleSolution(Route[]))
            feasible =
                feasibility(instance, ds = ds, cs = cs, solution = SimpleSolution(Route[]))
...
function delete_non_profitable_routes!(
    ...
    @showprogress "Delete non profitable routes " for t = instance.T:-1:1
