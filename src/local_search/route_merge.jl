...
function iterative_merge!(
    ...
    while true
        improvement_found, Δ =
            perform_best_merge!(instance, t, d, Δ, stats = stats, in_LNS = in_LNS)
...
function iterative_merge!(
    ...
    @showprogress "Greedy routes merge " for t = 1:instance.T, d = 1:instance.D
        iterative_merge!(instance, t, d, stats = stats, in_LNS = in_LNS)
    end
    verbose && println("Cost after merge : ", compute_cost(instance))
end
