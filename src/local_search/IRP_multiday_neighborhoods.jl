...
function change_day!(instance::Instance, route::Route, new_t::Int; stats::Dict = nothing)
    ...
    if new_t == t - 1
        ...
        if feasible
            oldcost =
                compute_cost(instance, ds = ds, cs = cs, ms = ms, ts = collect(new_t:T), solution = SimpleSolution([]))
    ...
    elseif new_t == t + 1
        ...
        if feasible
            oldcost = compute_cost(instance, ds = ds, cs = cs, ms = ms, ts = collect(t:T), solution = SimpleSolution([]))
            update_instance_some_routes!(instance, [route], "delete", false)
            update_instance_some_routes!(instance, [new_route], "add", false)
            newcost = compute_cost(instance, ds = ds, cs = cs, ms = ms, ts = collect(t:T), solution = SimpleSolution([]))
            update_instance_some_routes!(instance, [new_route], "delete", false)
            update_instance_some_routes!(instance, [route], "add", false)
        else
            newcost = Inf
...
"""
function iterative_change_day!(instance::Instance, direction::String = "ahead"; stats::Dict = nothing)
    improvement = true
    while improvement
